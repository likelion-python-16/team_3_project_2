...
def payment_page(request):
    ...
    context = {"plan": plan, "extend": extend, "user": request.user}
    ...
    if franchise_type:
        ...
        if franchise_count > 0:
            ...
            location_distribution = (
                franchise_cafes.values("distinct")
            ...
            trend_stats = trend_data.aggregate(
                ...
                investment_opportunities=Count(
            ...
            franchise_analysis = {
                ...
                "market_share": round((franchise_count / total_cafes) * 100, 1)
                if total_cafes > 0
                else 0,
                "estimated_monthly_sales": estimated_monthly_sales,
                "roi_prediction": round(
                    15.2 + (trend_stats["avg_growth_rate"] or 0) * 0.3, 1
                ),  # ROI 예측값
            }
...
def account_page(request):
    ...
    context = {"user": request.user, "profile": profile, "payments": payments}
...
def payment_success_api(request):
    """결제 성공 처리 API"""
    print(
        f"[DEBUG] API 호출 시작 - User authenticated: {request.user.is_authenticated}"
    )
    ...
    try:
        ...
        print(
            f"[DEBUG] 파라미터 추출 - paymentKey: {payment_key}, "
        ...
        if not payment_key or not order_id or not amount:
            missing_params = []
            if not payment_key:
                missing_params.append("paymentKey")
            if not order_id:
                missing_params.append("orderId")
            if not amount:
        ...
        if payment_key.startswith("tgen_"):
            print("[DEBUG] 테스트 결제로 인식 - 토스 API 검증 생략")
            # 테스트 환경에서는 검증 생략
            payment_data = {
                "method": "테스트결제",
                "orderId": order_id,
            ...
            # 실제 결제일 경우 토스 API 검증
            print("[DEBUG] 실제 결제 - 토스 API 검증 시작")
            ...
            if response.status_code != 200:
                print(
                    f"[DEBUG] 토스 API 응답 오류: {response.status_code}, "
                    f"{response.text}"
                )
                return JsonResponse(
                    {
                        "success": False,
                        "error": f"결제 검증에 실패했습니다. "
            ...
            if (
                ...
                or payment_data["totalAmount"] != amount
            ):
                print(
                    "[DEBUG] 결제 정보 불일치 - "
                    f"API orderId: {payment_data['orderId']}, "
                    f"요청 orderId: {order_id}"
                )
                print(
                    "[DEBUG] 결제 정보 불일치 - "
                    f"API amount: {payment_data['totalAmount']}, "
                    f"요청 amount: {amount}"
                )
                return JsonResponse(
        ...
        if existing_payment:
            return JsonResponse(
                {"success": False, "error": "이미 처리된 결제입니다."}, status=400
            )
        ...
        print(
            f"[DEBUG] 추출된 구독 개월 수: {subscription_months}, "
            f"orderName: {order_name}"
        )
