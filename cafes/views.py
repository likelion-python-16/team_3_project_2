...
class CafeIdViewSet(viewsets.ModelViewSet):
    ...
    def franchise_cafe_count_analysis(self, request):
        ...
        avg_growth = trends.aggregate(avg_growth=Avg('predicted_growth_rate'))['avg_growth'] or 0
    ...
    def franchise_type_analysis(self, request):
        ...
        # 브랜드 다양성 계산
        brand_diversity = cafes.filter(
            franchise=True, 
            franchise_type__isnull=False
        ).exclude(franchise_type='').values('franchise_type').distinct().count()
        
        # 상위 프랜차이즈 브랜드
        top_franchises = cafes.filter(
            franchise=True, 
            franchise_type__isnull=False
        ).exclude(franchise_type='').values('franchise_type').annotate(
            count=Count('cafe_id')
        ...
        # 시장 점유율 계산
        total_count = cafes.count()
        franchise_market_share = round((franchise_count / total_count) * 100, 1) if total_count > 0 else 0
        individual_market_share = round((individual_count / total_count) * 100, 1) if total_count > 0 else 0
    ...
    def franchise_sales_analysis(self, request):
        ...
        low_sales_stores = total_count - high_sales_stores - mid_sales_stores - low_mid_sales_stores
    ...
    def franchise_growth_analysis(self, request):
        ...
        for franchise_type in franchise_types[:8]:  # 상위 8개
            ...
            brand_growth = franchise_trends.aggregate(
                avg_growth=Avg('predicted_growth_rate')
            )['avg_growth'] or 0
            
            growth_leaders.append({
                'brand_name': franchise_type,
                'growth_rate': round(brand_growth, 1)
            })
        
        # 성장률별 정렬
        growth_leaders = sorted(growth_leaders, key=lambda x: x['growth_rate'], reverse=True)
    ...
    def trend_three_year_growth_analysis(self, request):
        ...
        if region and region != '서울시 전체':
            if '강남' in region or '서초' in region:
                growth_drivers = '프리미엄 시장 확산, 오피스 카페 증가, 고급 브랜드 확장'
    ...
    def trend_new_business_analysis(self, request):
        ...
        investment_opportunities = trends.filter(investment_opportunity=True).count()
        ...
        success_rate = round(((total_trends - risk_areas) / max(1, total_trends)) * 100, 1)
        ...
        for area in safe_areas:
            preferred_areas.append({
                ...
                'new_count': max(1, int(area['count'] * 0.1))  # 해당 지역 카페의 10%
            })
    ...
    def trend_survival_rate_analysis(self, request):
        ...
        five_year_survival = round(((high_recommendation + medium_recommendation * 0.7) / total_trends) * 100, 1)
        
        # 유형별 생존율
        franchise_cafes = cafes.filter(franchise=True)
        individual_cafes = cafes.filter(franchise=False)
        ...
        high_rent_risk = round((low_recommendation / total_trends) * 80, 1)  # 임대료는 추천도와 상관
    ...
    def trend_sales_index_analysis(self, request):
        ...
        sales_index = int(avg_recommendation * 24)  # 3레벨 = 72, 4레벨 = 96, 5레벨 = 120
        ...
        if region and region != '서울시 전체':
            ...
            for comp_region in comparison_regions[:3]:
                if comp_region != region:
                    ...
                    regional_index.append({
                        'region': comp_region,
                        'index': max(50, min(150, comp_index))
                    })
        else:
            # 서울시 전체 선택시 구별 비교
            ...
            for district in districts:
                ...
                regional_index.append({
                    'region': district,
                    'index': max(50, min(150, dist_index))
                })
