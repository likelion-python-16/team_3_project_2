...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--skip-cafe-id',
            action='store_true',
            help='Skip importing CafeId data'
        )
        parser.add_argument(
            '--skip-cafe-sales',
            action='store_true',
    ...
    def import_cafe_id(self, file_path):
            ...
            with open(file_path, 'r', encoding=encoding) as csvfile:
                ...
                with transaction.atomic():
                    for row_num, row in enumerate(reader, start=2):
                        try:
                            cafe_id = CafeId(
                                name=row['name'].strip() if row['name'] else f'Cafe_{row_num}',
                                city=row.get('city', '서울특별시').strip(),
                                distinct=row['distinct'].strip() if row['distinct'] else '',
                            ...
                            if success_count % 100 == 0:
                                self.stdout.write(f'Processed {success_count} CafeId records...')
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'CafeId import completed: {success_count} success, {error_count} errors'
            )
        )
    ...
    def import_cafe_sales(self, file_path):
                    ...
                    for row_num, row in enumerate(reader, start=2):
                        try:
                            ...
                            try:
                                ...
                                error_count += 1
                                self.stdout.write(
                                    self.style.WARNING(f'Row {row_num}: Cafe with ID {cafe_id} not found')
                                )
                                continue
                            ...
                            avg_price = int(sales / visitor_count) if visitor_count > 0 else 0
                            ...
                            if success_count % 100 == 0:
                                self.stdout.write(f'Processed {success_count} CafeSales records...')
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f'CafeSales import completed: {success_count} success, {error_count} errors'
            )
        )