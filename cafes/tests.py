...
from .models import CafeId, CafeSales, ResidentPopulation
...
class CafeModelsTest(TestCase):
    ...
    def test_cafe_sales_creation(self):
        ...
        sales = CafeSales.objects.create(
            cafe=self.cafe, date="2024-01", price=5000, visitor_count=100, sales=500000
        )
    ...
    def test_cafe_sales_aov_with_zero_visitors(self):
        ...
        sales = CafeSales.objects.create(
            cafe=self.cafe, date="2024-02", price=5000, visitor_count=0, sales=0
        )
    ...
    def test_cafe_sales_date_validation(self):
        ...
        with self.assertRaises(ValidationError):
            sales = CafeSales(
                cafe=self.cafe, date="2024/01", sales=1000, visitor_count=10, price=100
            )
            sales.full_clean()
        ...
        with self.assertRaises(ValidationError):
            sales = CafeSales(
                cafe=self.cafe, date="1999-12", sales=1000, visitor_count=10, price=100
            )
            sales.full_clean()
        ...
        sales = CafeSales(
            cafe=self.cafe, date=valid_date_str, sales=1000, visitor_count=10, price=100
        )
        try:
    ...
    def test_unique_cafe_date_constraint(self):
        ...
        CafeSales.objects.create(
            cafe=self.cafe, date="2024-03", price=5000, visitor_count=100, sales=500000
        )
